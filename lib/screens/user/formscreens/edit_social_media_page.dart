...
class EditSocialMediaPage extends StatefulWidget {
  ...
  const EditSocialMediaPage({super.key, required this.user});
...
class _EditSocialMediaPageState extends State<EditSocialMediaPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Medya Hesaplarım Düzenle'),
      ),
      body: BlocConsumer<ProfileBloc, ProfileState>(
        listener: (context, state) {
          if (state is ProfileLoaded) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Medya hesapları başarıyla güncellendi')),
            );
                    ...
                    child: Expanded(
                      child: ListView(
                        children: [
                          ...
                          const SizedBox(height: 16),
                          const Text('Eklenen Medya Hesapları',
                              style: TextStyle(
